...
helpers do
  def highlight(lang, &block)
    formatter = Rouge::Formatters::HTML.new(wrap: false)
    lexer = eval("Rouge::Lexers::#{lang}.new")

    content = capture(&block)
    out = '<pre class="highlight #{lang}">' + formatter.format(lexer.lex(content)) + '</pre>'
    concat out
  end
  ...
  def get_gravatar(email, size=128)
    require 'digest/md5'
